...
\section{Analyse cinématique générale}
...
\subsection{Mouvement contraint à l'axe des $x$}
\label{sec:x-bound}
\todo{plugger la partie à Shawn (raisonnement qui nous ammène à $\varphi=-\theta$)}
...
\begin{align}
  ...
  A_x(\theta)         = l_1\cos{\theta}+l_2\cos(-\theta) \\
	A_y(\theta,\varphi) = l_0+l_1\sin{\theta}+l_2\sin{\varphi} &\Rightarrow
  A_y(\theta) = l_0+l_1\sin{\theta}+l_2\sin(-\theta)
\end{align}
...
\begin{align}
  ...
  V_{A_y}(t) &=
      l_1\cos(\theta(t))\cdot\dot{\theta}(t)
    + l_2\cos(-\theta(t))\cdot\dot{-\theta}(t)
\end{align}

\todo{Décrisser le temps pour avoir tout en fonction de theta}

\subsection{Mouvement contraint à l'axe des $y$}
\todo{plugger la partie à Shawn}

Dans ce cas de figure, la logique est semblable à celle employée en
\ref{sec:x-bound}, à l'exeption que la relation (et ce qui en découle) est
maintenant:

\begin{align}
	\varphi &= \arccos(1-\cos\theta)
\end{align}
\todo{Dériver la grosse marde en haut}
...
\begin{multicols}{2}
	...
	\todo{Update l'image avec les bonnes variables du guide etudiant:
	  $C_B$, $F_B$
	}
	\begin{figure}[H]
		\centering
...
\begin{align}
  \vec{P_l_2} &= m_{BA} \cdot \vec{\alpha_g}
    = m_{BA} \cdot \begin{bmatrix} 0 \\ -9.81 \\ 0 \end{bmatrix}
    = \begin{bmatrix} 0 \\ -9.81 m_{BA} \\ 0 \end{bmatrix} \\
  ...
  \vec{F_I} &=
    ...
    \begin{bmatrix} 0 \\ 9.81 m_A \\ 0 \end{bmatrix} \\
  \vec{F_I} &=
    \begin{bmatrix} 0 \\ 9.81(m_{BA}+ m_A) \\ 0 \end{bmatrix} \\
\end{align}
...
\begin{align}
  ...
  \vec{M_l_2} &= (\vec{BA}/2)\times\vec{P_A}
    = \begin{bmatrix} \cos{l_2}/2 \\ \sin{l_2}/2\\ 0 \end{bmatrix}
      \times \begin{bmatrix} 0 \\ -9.81 m_{BA} \\ 0 \end{bmatrix} \\
    &=  \vec{i} \begin{vmatrix} \sin{l_2}/2 & 0           \\ -9.81m_{BA} & 0          \end{vmatrix}
      - \vec{j} \begin{vmatrix} \cos{l_2}/2 & 0           \\ 0          & 0          \end{vmatrix}
      + \vec{k} \begin{vmatrix} \cos{l_2}/2 & \sin{l_2}/2 \\ 0          & -9.81m_{BA} \end{vmatrix} \\
    &= 0\vec{i} - 0\vec{j} + \frac{-9.81m_{BA}\cos{l_2}}{2}\vec{k}
      = \begin{bmatrix} 0\\0\\-(9.81m_{BA}\cos{l_2})/2 \end{bmatrix} \\
  \sum{\vec{M}} &= \vec{M_A} + \vec{M_l_2} + \vec{M_B} = \vec{0} \\
  \vec{0} &= \begin{bmatrix} 0\\0\\-9.81m_A\cos{l_2}\end{bmatrix}
    + \begin{bmatrix} 0\\0\\-(9.81m_{BA}\cos{l_2})/2 \end{bmatrix}
    + \vec{M_B} \\
  \vec{M_B} &= - \begin{bmatrix} 0\\0\\-9.81m_A\cos{l_2}\end{bmatrix}
    - \begin{bmatrix} 0\\0\\-(9.81m_{BA}\cos{l_2})/2 \end{bmatrix} \\
    &= \begin{bmatrix} 0\\0\\9.81m_A\cos{l_2}\end{bmatrix}
      + \begin{bmatrix} 0\\0\\(9.81m_{BA}\cos{l_2})/2 \end{bmatrix} \\
        &= \begin{bmatrix} 0\\0\\9.81(m_A\cos{l_2}+m_{BA}\cos{l_2}/2) \end{bmatrix}
\end{align}
...
\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{figures/x-bound-pos.pdf}
	\caption{$A_x$ selon l'angle de la tige $\vec{OB}$}
	\label{fig:x-bound-pos}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{figures/x-bound-v.pdf}
	\caption{$V_A_x$ selon l'angle de la tige $\vec{OB}$}
	\label{fig:x-bound-v}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{figures/x-bound-alpha.pdf}
	\caption{$\alpha_A_x$ selon l'angle de la tige $\vec{OB}$}
